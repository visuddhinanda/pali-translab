...
5. 义理或用词与注释书不符
6. 代词指代错误
7. **文言 / 半文半白语体**：出现文言虚词（之/其/乃/便/即/谓/於/遂…）、文言句式（…者…也／于…之际／经…后／答以…）、或单音节文言词（食用→吃、用膳→吃、端坐、盛装、年方…）。**按 error 处理**：每处套 span（类别填「文言语体」），并在理由的"风格"维度据此显著扣分。译文须为现代书面汉语，详见 `knowledge/style.md` 第一节禁例表。
...
```json
{"id": "94-1-2-16", "pali": "Bhagavato ...", "zh": "礼敬彼<span class='evaluate-error' title='🟧-词义错误：“彼”非现代汉语｜建议：改为“那位”'>彼</span>世尊、阿罗汉、正等正觉者", "confidence": 95}
```
